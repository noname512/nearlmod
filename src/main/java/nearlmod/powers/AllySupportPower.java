...
public class AllySupportPower extends AbstractPower implements CloneablePowerInterface {
    ...
    public AllySupportPower(AbstractCreature owner, int amount) {
        ...
        ID = POWER_ID;
        this.owner = owner;
        region128 = new TextureAtlas.AtlasRegion(ImageMaster.loadImage("resources/nearlmod/images/powers/allysupport power 84.png"), 0, 0, 84, 84);
        region48 = new TextureAtlas.AtlasRegion(ImageMaster.loadImage("resources/nearlmod/images/powers/allysupport power 32.png"), 0, 0, 32, 32);
        type = PowerType.BUFF;
        this.amount = amount;
        updateDescription();
    }
    ...
    public void onAttack(DamageInfo info, int damageAmount, AbstractCreature target) {
        ...
        for (AbstractOrb orb : ((AbstractPlayer)this.owner).orbs)
            if (orb instanceof Fartooth) {
                addToBot(new DamageAction(target, new DamageInfo(null, amount + ((Fartooth) orb).getTrustAmount(), DamageInfo.DamageType.THORNS)));
                break;
            }
    }
